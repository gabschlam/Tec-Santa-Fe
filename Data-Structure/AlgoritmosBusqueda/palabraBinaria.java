/*
    Algoritmos de Búsqueda: Búsqueda de Palabras de forma Binaria
    Estructura de Datos
...
public class palabraBinaria
{
	...
	{	
		...
		res = busquedaBinaria(arreglo, palabra);

		if (res >= 0) 
		{
			System.out.println("La palabra si existe en el arreglo, esta en la posicion: " + res);
		} 
		else 
		{
	...
	public static int busquedaBinaria(String[] arreglo, String palabra) 
	{
		...
		while (inf < sup)
		{
			...
			if (palabra.compareTo(arreglo[medio]) < 0) 
			{
				sup = medio;
			}

			else if (palabra.compareTo(arreglo[medio]) > 0)
			{
				inf = medio + 1;
			}
			else 
	...
	/*public static int busquedaBinaria(String[] arreglo, String palabra, int inf, int sup)
	{
		Arrays.sort(arreglo);

		int mitad = (inf + sup)/2;

		if(arreglo[mitad].compareTo(palabra) == 0)
		{
			return mitad;
		}

		else if(arreglo[mitad].compareTo(palabra) > 0)
		{
			return busquedaBinaria(arreglo, palabra, inf, mitad-1);
		}

		else if(arreglo[mitad].compareTo(palabra) < 0)
		{
			return busquedaBinaria(arreglo, palabra, mitad+1, sup);
		}
